<template>
    <div class="flex h-screen flex-col items-center justify-center">
        <div class="absolute inset-x-0 top-0 flex z-0">
            <slot name="dealer"></slot>
        </div>
        <div class="absolute inset-x-0 bottom-0 flex z-0">
...
<script setup lang="ts">
    ...
    watch(() => gameStore.gameState, (newGameState) => {
        ...
        if(newGameState === 'player_win' || newGameState === 'dealer_win' || newGameState === 'push') {
            modal.value = true;
        }
    });
</script>
